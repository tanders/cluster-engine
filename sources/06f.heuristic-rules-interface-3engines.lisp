...
;       meter-note heuristic rules in one voice.
;


(defun heuristic-switch-rule-3-engines-metric-timepoints-events-and-pitch-include-rests-indicate-duration (simple-rule rhythm-engine2 pitch-engine3 flag-for-metric-access weight)
...
(defun heuristic-switch-rule-3-engines-metric-timepoints-events-and-pitch-include-rests-indicate-duration-and-meter (simple-rule rhythm-engine2 pitch-engine3 flag-for-metric-access weight)
  "Formats a heuristic switch rule for how events (i.e. durations or rests) are positioned at metric points. Grace notes are ignored. 
...
The logic statement will receive '(offset duration pitch meter).
"
  (when (/= (1+ rhythm-engine2) pitch-engine3) (error "Error in meter-note-rule: rhythm-engine and pitch engine has to be from the same voice."))
  (let ((no-of-args (length (function-lambda-list simple-rule)))
        (metric-timepoints1 (cond ((= flag-for-metric-access 1) ; all beats
...
(defun heuristic-switch-rule-3-engines-metric-timepoints-events-and-pitch-include-rests-indicate-duration-list-all (simple-rule rhythm-engine2 pitch-engine3 flag-for-metric-access weight)
  "Formats a heuristic switch rule for how events (i.e. durations or rests) are positioned at metric points. Grace notes are ignored. 
    ...
    (list 'lambda '(vsolution vlinear-solution vindex engine nth-candidate)
          ...
          (list 'block 'this-rule
                ...
                (list 'let '((metric-engine1 (1- (the fixnum (array-dimension vindex 0))))
                      ...
                      ;here is the rule test
                      (list 'if (list 'funcall (compile-if-not-compiled nil simple-rule) 'list-of-variables-to-test)
                            weight
                            0)
                      )))
    ))
...
;;;;;;;;;;;; note-meter heuristic switch rules ;;;;;;;;;;;;

(defun heuristic-switch-rule-3-engines-notes-and-their-offset-to-metric-structure-include-rests-indicate-timesignature (simple-rule rhythm-engine1 pitch-engine3 flag-for-metric-access weight)
  "Formats a heuristic switch rule for offsets for notes (including pitches and rests), i.e. how notes are positioned in relation to the pulse.
...
(defun heuristic-switch-rule-3-engines-notes-and-their-offset-to-metric-structure-include-rests-ignor-gracenotes-indicate-timesignature (simple-rule rhythm-engine1 pitch-engine3 flag-for-metric-access weight)
  "Formats a heuristic switch rule for offsets for notes (including rests), i.e. how notes are positioned in relation to the pulse.
The rule will also access the event ratio (= length), the timesignature for the measure where the events onset exist and the pitch. 
...
(defun heuristic-switch-rule-3-engines-notes-and-their-offset-to-metric-structure-ignor-rests-indicate-timesignature (simple-rule rhythm-engine1 pitch-engine3 flag-for-metric-access weight)
...
(defun heuristic-switch-rule-3-engines-notes-and-their-offset-to-metric-structure-ignor-rests-and-gracenotes-indicate-timesignature (simple-rule rhythm-engine1 pitch-engine3 flag-for-metric-access weight)
...
(defun heuristic-rule-three-engines (rule engine1 engine2 engine3)
